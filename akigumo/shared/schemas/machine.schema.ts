...
export const successEvent = <T extends string, D extends z.ZodTypeAny>(
	type: T,
	dataSchema: D,
) => z.object({ type: z.literal(type), payload: dataSchema });

export const failureEvent = <T extends string>(type: T) =>
	z.object({ type: z.literal(type), payload: ErrorDetailSchema });
 ...
 */
export function buildWorkflowError(eventPayload: unknown, path?: string) {
	const e = eventPayload as any;
	const message =
