...
import { ONTOLOGY_NAMESPACE } from "../contracts/index.js";
import { FILE_CATEGORY_DEFS } from "./file.constants.js";
...
async function getCategoryByCode(code: string) {
	...
	if (!category) {
		throw new Error(
			`CRITICAL_DATA_MISSING: FileCategory with code '${code}' not found in database. Please run seed tasks.`,
		);
	}
	return category;
}
		...
		await prisma.$transaction(async (tx) => {
			for (const cat of Object.values(FILE_CATEGORY_DEFS)) {
				...
				for (const ext of cat.extensions) {
					...
					const conceptId = uuidv5(ext.code, ONTOLOGY_NAMESPACE);
		...
		async () => {
			...
			if (mimeType) {
				...
				if (category) {
					...
					// Fallback 到 OTHER
					const otherCat = await getCategoryByCode(
						FILE_CATEGORY_DEFS.OTHER.code,
					);
			...
			return { extensionId: ext.id, conceptId: concept.id };
		},
	);
}
