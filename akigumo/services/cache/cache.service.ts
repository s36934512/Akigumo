...
const DEFAULT_REDIS_CACHE_URL =
	process.env.REDIS_CACHE_URL ?? "redis://redis-cache:6379";
...
const SingleParamSchema = z.discriminatedUnion("module", [
	z.object({
		...
		key: z.uuid(),
		ttl: z.number().default(3_600_000),
	}),
	z.object({
		module: z.literal("Item"),
		key: z.uuid(),
		ttl: z.number().default(3_600_000),
	}),
	z.object({
		module: z.literal("ItemFileInfo"),
		key: z.string(),
		ttl: z.number().default(3_600_000),
	}),
	z.object({
		module: z.literal("Concept"),
		key: z.uuid(),
		ttl: z.number().default(3_600_000),
	}),
	z.object({
		module: z.literal("User"),
		key: z.uuid(),
		ttl: z.number().default(1_800_000),
	}),
	z.object({
		module: z.literal("Auth"),
		key: z.string().min(10),
		ttl: z.number().default(600_000),
	}),
]);
...
const InvalidateSchema = z
	...
	.transform((val) => {
		...
		if ("keys" in val)
			return val.keys.map((k) => ({ module: val.module, key: k }));
		return [val];
	});
...
export const cacheService = {
	...
	async get<T>(param: CacheParam): Promise<T | undefined> {
		const validated = SingleParamSchema.parse(param);
		return (
			(await cache.get<T>(this.getFullKey(validated.module, validated.key))) ??
			undefined
		);
	},
	...
	async set<T>(param: CacheParam, value: T): Promise<void> {
		const validated = SingleParamSchema.parse(param);
		await cache.set(
			this.getFullKey(validated.module, validated.key),
			value,
			param.ttl ?? validated.ttl,
		);
	},
	...
	async invalidateMany(input: InvalidateInput): Promise<void> {
		...
		const fullKeys = items.map(({ module, key }) =>
			this.getFullKey(module, key),
		);
		await this.executeBatchDelete(fullKeys);
	},
};
