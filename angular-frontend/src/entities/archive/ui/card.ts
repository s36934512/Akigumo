...
import type { ArchiveEntry } from "../model";

@Component({
	selector: "entity-archive-item-card",
	standalone: true,
...
export class ItemCardComponent {
	entry = input.required<ArchiveEntry>();
	isHovered = input(false);

	cardMouseEnter = output<void>();
	cardMouseLeave = output<void>();
	...
	toSubtitle(): string {
		const entry = this.entry();
		if ("count" in entry) return `ITEMS_${entry.count}`;
